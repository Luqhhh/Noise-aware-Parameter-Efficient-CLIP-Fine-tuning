...
状态：**D0、冻结 smoke、工程诊断、修复后的 24,967 行 L1 全量重放及 R0/R1 交付已完成；两个候选均等待真实平台回填。**
...
平台尚未自动上传。待依次回填 R0、R1 百分比后，与 L1 无 prior 69.2794% 比较并按固定停止规则保留胜者；精确正确数与实际上传时间保持空值，不作反推。72.4677% 仍是不同来源协议的 G0+legacy test-batch prior0.9 单列绝对最高。
