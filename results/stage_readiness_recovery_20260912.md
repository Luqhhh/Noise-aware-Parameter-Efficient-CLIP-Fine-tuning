...
该 61.4010% 负结果、命令、指标和各关键哈希继续由本文及提交登记表保存。完整恢复路线已从当前计划移出，`outputs/stage_readiness/20260912_full_rebuild_r1/` 的 34,563,599,325 bytes 本地输出（含多轮 checkpoint、缓存和恢复包副本）已按用户要求删除。被删字节未纳入 Git，不能从本仓库直接恢复；如再次需要，只能按历史固定流程重新生成。当前 v5 胜者及其晋级父链未受此次删除影响。
