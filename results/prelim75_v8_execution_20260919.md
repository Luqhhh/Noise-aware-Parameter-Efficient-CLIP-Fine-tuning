...
状态：**B0 平台 67.8816%，为负结果并淘汰；B1 已提交就绪但尚待平台回填，本轮尚未闭环。**
...
B0 − G0 无 prior = **−1.3458pp**；B0 − L1 无 prior = **−1.3978pp**；B0 相对 72.4677% 的不同来源协议绝对最高低 **4.5861pp**，距 75% 为 **7.1184pp**。B0 不再作为非测试拟合路径胜者候选。用户只提供百分比，精确正确数与实际上传时间保持未知。
...
B0 已证明该训练源偏置在 G0 上造成平台下降，不能以数值目标下降或边际更均衡替代平台证据。不改变配方、不派生扫描；等待 B1 回填后计算 B1−69.2794 与 B1−B0，并按预注册止损规则关闭本轮。72.4677% 仍是不同来源协议的已报告绝对最高。
