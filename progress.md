...
- 配置 `configs/prelim75_v2.yaml`；main上实现H0/H1共享head重拟合、V0/V1 global/local短微调及固定无校准交付。完整P的103218行十视图缓存完成，初始logits重构差0。
- H0/H1已完成固定10epoch及24967行CSV/ZIP校验，V0/V1均已完成3epoch及同协议交付；包位于 `outputs/prelim75_v2_20260912/{H0,H1,V0,V1}/submission/`，桌面有同哈希副本。
- H0平台真实回填 **65.8629%**，低于匹配无校准P66.7681%共0.9052pp，不晋级。H1平台66.1914%，低于P0.5767pp；H0/H1均未超过P，H及C关闭。V0平台67.4090%，V1平台67.5932%；V1比V0高0.1842pp、比同协议P高0.8251pp，最终保留V1。V1比历史70.352866%低2.759666pp，距75分仍差7.4068pp，本轮结束。历史最佳70.352866%保留，不将恢复R1或新本地指标替代历史记录。
- V1已完成训练/真实诊断/提交，local-only loss对视觉投影和O3/PTA梯度均非零，终态冻结参数不变，batch32未降档。C原门槛未满足，本轮取消；不追加H拟合/扫描。V路线已由四组真实平台成绩确定最终胜者，不用本地重叠诊断替代；不追加训练、prior拟合或扫描。
- 实际命令、源码和产物哈希见 [执行记录](results/prelim75_v2_execution_20260912.md)。H/V本地指标是10316行重叠诊断，不独立验证平台泛化。只本地提交，不push，不操作平台上传账号。
