...
## Balanced-prior 校准：prior 1.0 平台 67.2007%，距 70 分仅 2.80pp

- **prior 强度 0.25 → 1.0**（W060 checkpoint + M1/Flip）平台实测 **67.2007%**，
  相对 0.25 包（65.5786%）再 `+1.6221pp`，相对无校准 M1/Flip（63.7802%）
  `+3.4205pp`。strength 1.0 确认最优（IPF 完全拟合均衡先验）。
- 累计校准收益：local→platform gap 由 8.35pp 压缩到 5.08pp，距离 70 分
  `2.80pp`。
- 模型测试预测严重不均衡（校准前最差类 2 个、最好类 ~180 个预测），平台测试
  类别均衡 —— 两个假设都被平台分数验证。
- 校准只调整推理 logits，可叠加任何新 checkpoint；与"更多数据"训练方向
  （trust 阈值 0.70→0.60→0.50，M1+flip raw 72.13→72.20→72.28%）正交叠加。
