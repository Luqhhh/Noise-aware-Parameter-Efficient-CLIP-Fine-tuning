...
## Balanced-prior 校准：平台 65.5786%，新最佳，+1.80pp 零训练成本

- F1 REBUILD R1 + M1/Flip 包叠加 `align_logits_to_prior` 均衡先验校准
  （strength 0.25，IPF 类别偏置拟合），平台实测 **65.5786%**，相对无校准包
  `+1.7984pp`，是新的审计完整平台最佳。
- 直接验证两个假设：平台测试类别均衡；模型测试预测严重不均衡（校准前最差类
  仅 2 个预测、最好类 190 个，均衡期望 ~50/类）。
- 校准只调整推理 logits（不训练、不改模型），local→platform gap 由 8.35pp
  收窄至 6.55pp；距离 70 分 `4.42pp`。
- 该信号可叠加任何新 checkpoint，是独立于训练的上行方向；下一步扫描更高强度
  `{0.5, 0.75, 1.0}`。
