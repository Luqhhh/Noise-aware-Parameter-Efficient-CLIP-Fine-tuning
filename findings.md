...
- **全微调 R3MS 父模型 + 双 Adapter（O3 BN32 + PTA BN64）+ balanced-prior
  0.90** 平台实测 **70.352866%（17,565/24,967）**，新的审计完整平台最佳。
- 完整曲线：父模型基线 69.575840% → 双 Adapter 基线 70.256739%（首个突破
  70%）→ pa0.89 70.344855% → **pa0.90 70.352866%** → pa0.91 70.308808% →
  pa0.92 70.340850% → pa0.93 70.328834%；**prior 峰值确认为 0.90**。
- **关键机制：LoRA 自训练父模型 → 全微调父模型迁移带来 +0.50pp 级平台增益
  （69.075179% → 69.575840%），双 Adapter 再 +0.68pp；全微调父模型是当前
  最大的单一杠杆。** 双 Adapter 与 prior 校准叠加有效，但 prior 维度边际已尽。
- 桌面工作流（用户指示）：桌面只保留 `submission.zip` 一个提交包（当前
  pa0.94 待平台验证），其余候选归档于 `outputs/delivery/`。
- 权威交付状态见 `outputs/delivery/README_delivery_status.md`。
