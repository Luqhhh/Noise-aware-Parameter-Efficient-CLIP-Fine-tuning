...
## Balanced-prior 校准：strength 0.75 平台 67.2848%，距 70 分仅 2.72pp

- **强度曲线实测 0.25 / 0.75 / 1.0，strength 0.75 最优**：W050+0.75 =
  **67.2848%** > W060+1.0 = 67.2007% > W050+1.0 = 67.1607% > 0.25 = 65.5786%。
- 关键发现：**1.0 过矫正**——测试集并非完美均衡（24,967 = 467 类 × 50 +
  33 类 × 49），完全拟合 uniform 会轻微过度校准。用户坚持测中间值被平台证实。
- 累计校准收益：local→platform gap 由 8.35pp 压缩到 ~5.0pp，距离 70 分
  `2.72pp`。
