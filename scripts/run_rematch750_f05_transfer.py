#!/usr/bin/env python3
"""Materialize device-bound configs and optionally run the F05 transfer sweep."""
...
TRIALS = tuple(f"ET{index:02d}" for index in range(1, 11))
...
def _manifest_trials() -> dict[str, dict]:
    path = ROOT / "configs" / "rematch750_f05_transfer" / "manifest.json"
    manifest = json.loads(path.read_text(encoding="utf-8"))
    trials = {trial["name"]: trial for trial in manifest["trials"]}
    if tuple(trials) != TRIALS:
        raise ValueError("manifest trials differ from the runner declaration")
    return trials
...
def main() -> int:
    ...
    runtime_dir = (
        ROOT / "outputs" / "xjn" / "rematch750_f05_transfer" / "_runtime_configs"
    )
