...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="train",
        help="Root directory containing class subdirectories with images.",
    )
    parser.add_argument(
        "--output-root",
        type=str,
    ...
    json.dump(
        class_to_idx,
        (output_dir / "class_to_idx.json").open("w"),
        indent=2,
        sort_keys=True,
    )
    json.dump(
        idx_to_class,
        (output_dir / "idx_to_class.json").open("w"),
        indent=2,
        sort_keys=True,
    )

    df["class_idx"] = df["class_name"].map(class_to_idx)
    missing = df["class_idx"].isna().sum()
    if missing:
        raise ValueError(
            f"{missing} images have class names not in class_to_idx mapping."
        )
    ...
    train_df[["image_path", "class_name", "class_idx"]].to_csv(
        train_csv, index=False
    )
    val_df[["image_path", "class_name", "class_idx"]].to_csv(
        val_csv, index=False
    )
    ...
    if overlap:
        raise AssertionError(
            f"Train/val overlap detected: {len(overlap)} images"
        )

    union = train_paths | val_paths
    all_count = len(df)
    if len(union) != all_count:
        raise AssertionError(
            f"Union of train+val ({len(union)}) != total images ({all_count})"
        )

    # Verify class coverage
    train_classes = set(train_df["class_idx"].unique())
    val_classes = set(val_df["class_idx"].unique())
    if len(train_classes) != df["class_name"].nunique():
        logger.warning("Train split missing some classes!")
    if len(val_classes) != df["class_name"].nunique():
        logger.warning("Val split missing some classes!")
    ...
    manifest = {
        ...
        "source_file_count": int(all_count),
    ...
    logger.info(
        "Master split created: %d train / %d val / %d classes",
        len(train_df),
        len(val_df),
        df["class_name"].nunique(),
    )
