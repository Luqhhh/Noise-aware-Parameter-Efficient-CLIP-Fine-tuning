#!/usr/bin/env python3
"""Materialize device-bound configs and optionally run the WD factorial.

The tracked configs deliberately use ``npu:UNASSIGNED``.  This helper requires
the operator to expose an idle physical NPU, binds the process-local logical
device, and retains the exact runtime YAML beside the experiment outputs.
"""
...
def main() -> int:
    ...
    output_root = ROOT / "outputs" / "xjn" / "rematch750_wd_relax"
    runtime_dir = output_root / "_runtime_configs"
    commands: list[list[str]] = []
    for trial in args.trials:
        source = ROOT / "configs" / "rematch750_wd_relax" / f"{trial}.yaml"
