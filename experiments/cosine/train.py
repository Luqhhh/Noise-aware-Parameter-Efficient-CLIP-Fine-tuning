"""
Training script for the CLIP Cosine Classifier.

Trains a cosine-similarity classification head on top of frozen CLIP ViT-B/32
features. Supports AMP mixed-precision training, CosineAnnealingLR scheduling,
warmup, checkpointing, resume, and CSV logging.

Usage:
    python -m experiments.cosine.train --config configs/cosine.yaml
    python -m experiments.cosine.train --config configs/cosine.yaml --resume outputs/cosine/checkpoints/last.pt
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train CLIP Cosine Classifier"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to checkpoint to resume from (e.g., outputs/cosine/checkpoints/last.pt).",
    )
...
def _build_optimizer_and_scheduler(
    ...
    Uses the cosine classifier's get_param_groups() to handle the logit_scale
    parameter (if learnable) with a reduced learning rate and no weight decay.
    """
    train_cfg = config["train"]

    # Use model-defined param groups (handles logit_scale separately)
...
def train_one_epoch(
    ...
    for batch_idx, (images, labels, _paths) in enumerate(pbar):
        ...
        # Clamp logit_scale after each optimizer step (only relevant when learnable)
        model.clamp_scale()
