...
本地表格为完全重叠诊断，未作为epoch/参数搜索依据；H0真实平台65.8629%低于匹配无校准P66.7681%，不晋级。四组平台成绩均已真实回填，精确正确数/实际上传时间仍留空，75%未达到。H0/H1均低于匹配P，H路线及C已关闭，不追加拟合/扫描；V0平台67.4090%，V1平台67.5932%；V1比V0高0.1842pp、比同协议无校准P高0.8251pp，保留为本轮最终无校准胜者。V1比历史最好70.352866%低2.759666pp，距75仍差7.4068pp，历史最佳未刷新。本轮执行及成绩回填结束，不新增训练或prior拟合。历史70.352866%及原包保留，新训练不使用旧测试拟合prior，也不将历史校准生产或完整上游谱系认证写作完成。

工程交付证据、命令及全部哈希见 [执行记录](results/prelim75_v2_execution_20260912.md) 和 [视觉段记录](results/prelim75_v2_visual_20260913.json)。工程候选交付及四组平台回填验收通过，V1相对同协议无校准P的平台提分已验证；历史最高未刷新，75目标未达到。最终选择及包绑定见 [本轮最终记录](results/prelim75_v2_final_20260913.json)；main本地提交，不push。CPU Aegis451 passed/1 skipped（隔离GPU）、CUDA补测1 passed、根回归424 passed/1 skipped；实际CUDA数值/训练检查另有运行记录。
...
|---|---|---:|---|
| A12_CORR seed 42 | M1/Flip 0.40/0.50 + temp1.5 + balanced-prior 0.85 | **67.6853%** | `audited`（新平台最佳）：checkpoint/prediction/ZIP 哈希齐全 |
| A12 seed 42 | M1/Flip 0.40/0.50 + temp1.5 + balanced-prior 0.85 | 67.6173% | `audited`：checkpoint/prediction/ZIP 哈希齐全 |
