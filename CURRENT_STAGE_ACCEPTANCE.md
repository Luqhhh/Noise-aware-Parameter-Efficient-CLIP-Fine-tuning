...
本地表格为完全重叠诊断，未作为epoch/参数搜索依据；H0真实平台65.8629%低于匹配无校准P66.7681%，不晋级。其他平台字段留空，75%未认证。H0/H1均低于匹配P，H路线及C已关闭，不追加拟合/扫描；V0平台67.4090%，比同协议无校准P高0.6409pp，暂保留；距历史最好70.352866%仍低2.943866pp，历史最佳未刷新。V1平台尚待回填。历史70.352866%及原包保留，新训练不使用旧测试拟合prior，也不将历史校准生产或完整上游谱系认证写作完成。

工程交付证据、命令及全部哈希见 [执行记录](results/prelim75_v2_execution_20260912.md) 和 [视觉段记录](results/prelim75_v2_visual_20260913.json)。当前仅工程候选交付验收通过，平台提分和75目标尚待真实结果；main本地提交，不push。CPU Aegis451 passed/1 skipped（隔离GPU）、CUDA补测1 passed、根回归424 passed/1 skipped；实际CUDA数值/训练检查另有运行记录。
