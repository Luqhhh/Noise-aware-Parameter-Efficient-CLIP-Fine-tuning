...
data:
  ...
  split_seed: 42
  split_dir: outputs/data/d3_strict/seed42  # mandatory for all new experiments
...
最终提交必须来自单模型和单一推理流程。
   **2026-07-17 决议：所有新实验统一使用 `split_dir: outputs/data/d3_strict/seed42`。**
   `outputs/baselines/ref/seed42` 已弃用。原因：d3_strict 的 group-aware 分法防止同图跨 train/val 泄漏，
