...
2026-09-11 读取的[官网赛题页](https://www.aicomp.cn/tracks/tracks-1/3714.html)列出复赛 750 类、148,695 训练、37,444 测试；半决赛 500 类、90,197 训练、24,912 测试。此前用户计划/PDF来源记录分别为 1500/297282/74896 与 1000/180274/49857。2026-09-15 组委会正式通知已明确：为缓解复赛和半决赛周期限制，类别数分别减半为 750 和 500，因此类别数冲突已经解决。旧规模仅保留为历史来源记录或软件边界测试；训练/测试样本数仍按当前官网口径登记，并在正式数据到位后逐项核验。

## 尚未完成

R2 真实节点复现与恢复、R3 间接谱系/开发隔离、R4 类级真实监督账本、R5 校准来源绑定、R6 750/500 类当前规模演练、R7 完整技术材料/PDF/复现包、R8 干净环境完整训练均未在本段完成。类别数冲突已经解决，但正式数据清单仍须核验；不能把旧 1500/1000 类演练当成当前官方规模验收。
