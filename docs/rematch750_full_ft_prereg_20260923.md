...
**2026-09-23 机会成本更新（用户当日指令）**：上式是按本机算力稀缺时的口径写的。当时队友 NPU 单点 ≈34.3 分钟且可并行，本包算力机会成本 ≈0，故曾是「最便宜的一条关轴动作」。

**⚠️ 2026-09-26 再次更正（用户改回「自己想策略自己跑」）**：上一段的 NPU 前提**已撤回**。本包改按**本机**口径重算 —— FULL00（冻结特征线性探针，分钟级）+ FULL01（16 轮 + 11% 样本）≈ **2.8 小时串行**，且 16 轮档在本机有 **2/2 崩溃史**（`cudaErrorUnknown`）。因此本包的性价比**下降**：由「几乎免费地关一条轴」变成「2.8 小时 + 崩溃风险，换一个预期落在 ±0.20pp 空档的结论」。**跑不跑、要不要占名额由用户裁决。**
...
| 花掉一个平台名额换一个大概率空档的结论 | 已在 §2 明写机会成本；**是否值得由用户裁决**（本机只提供判据与候选，不做名额决策） |
