...
> 2. **更严重的是：尾部这个判据在结构上不可能影响平台成绩。** 固定训练尾部 75 类只占验证集 **1003/14,880 = 6.74%** 的图。因此：
>    - 晋级门 +0.20pp（尾部口径）→ 折算到整体准确率只有 **+0.0135pp**
>    - 队友 FT/LT 那 +2.57pp 的尾部增益 → 折算后 **+0.173pp**
>    - 而平台上真实发生的是 **−0.08pp（−30 张）**
>
>   也就是说：**这个门就算超额通过 15 倍，在平台上仍然是看不见的。** 判据被换成了"高分辨力但无后果"的指标 —— 分辨力本身不是目的，能量到平台分才是。
>
> **教训**：改判据时只问了「哪个指标的差值大」，没问「这个指标占测试集的多少」。前者是敏感度，后者是量级；**敏感度和重要性是两件事**，缺任何一个都会选错。（本条为事后补记，不改变已按原判据作出的关闭决定 —— 它使那个决定更站得住，不是翻案。）
