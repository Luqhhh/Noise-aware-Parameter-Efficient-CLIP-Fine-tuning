...
> **更正（2026-09-22，结果产出后补记）：上面这两句都错了，而且错在同一个地方。**
> 1. **「整体 macro 分辨力只有 ~0.03pp」是把「观测到的差值」当成了「噪声下限」。** 实测整体 macro 的**配对** SE 是 **0.0925pp**，即 2 SE ≈ 0.19pp —— 它完全能分辨 0.20pp。FT/LT 之间那 +0.03pp 是**两者真的几乎没差别**，不是测不出来。
> 2. **指标权重更正（REMATCH750_V2）：不得用验证图片占比换算尾部 macro 对类别均衡测试目标的贡献。** 固定训练尾部 75 类占 750 类的 10%，验证图片的 1003/14,880 = 6.74% 不是该宏观权重。
>    在「其余类别不变、增益能迁移」的假设下，ΔA = 0.1 × ΔA_75 + 0.9 × ΔA_675。尾部 +0.20pp 的假设贡献为 +0.020pp；FT/LT 尾部约 +2.566pp 的假设贡献为 +0.257pp，撤销原 +0.0135pp / +0.173pp 换算。
>    这是类别均衡目标的算术分解，不是平台收益预测。平台实际 LT 比 FT 低 0.0801pp / 30 张，必须同时检查其余 675 类变化与迁移差异，不能称尾部在结构上不可能影响平台。
>
> **教训**：整体 macro/micro 保留为主表，固定训练尾部只作解释，并同时报告其余 675 类。上述修正不改变 OOF 按原预注册判据失败、已关闭、无提交包、不派生扫描的决定。
...
**平台对照（同日新增证据，独立于本实验）**：队友回填的两次平台成绩见 [平台结果](rematch750_platform_results_20260922.md) —— RM_FT **60.9657%**（22,828/37,444）、RM_LT **60.8856%**（22,798/37,444），**RM_LT 低 0.08pp（−30 张）**。

这对本轮的意义是加强结论，不是翻案：RM_LT 在**本地整体 macro 上比 RM_FT 高 +0.03pp**，在平台上却反向低 0.08pp —— 说明本地 0.0x pp 量级的差异（无论整体还是尾部）**不具备平台可迁移性**。本实验的本地整体差值 −0.13pp、尾部 +0.01pp 都在这个量级内，即便当时判据通过，按同一逻辑也不该期待平台增益。本轮关闭因此不只是"没达标"，而是**这类指标口径下的正确决定**。

同时这也给下一轮提了要求：若下轮改以 bottom-10% macro（+0.44pp）为判据，必须先说明它比整体 macro 更可迁移的**理由**，而不是因为它在本地好看。平台数据目前只否证了整体 macro 的可迁移性，对 bottom-10% 尚无证据。

**独立复核**：队友 xjn 在 `main@b49f973` 上对本 sidecar 做过一次零 GPU 独立审计，各项数值与本文件一致（行数 133,815、折行数 26,763/26,764/26,763/26,763/26,762、权重 `[0.302216, 1.0]`、均值 0.628463、同意/不同意 0.705820/0.480729、尾部 75 类 0.612259、class 183 保持 1.0）。该审计所在的提交 `230893b` 随后被其自行 revert，故不在当前树中，仅作为独立复核证据记录。
