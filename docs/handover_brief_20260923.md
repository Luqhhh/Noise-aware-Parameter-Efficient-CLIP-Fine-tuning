...
**本 brief 由 clairvoyanttt 在其本机 `LAPTOP-G42LAJ37`（home `/home/clairvoyant`）上写。** 而你的 V3 方案文档记录你的本机 worktree 在 `/home/lux1/noise-worktrees/...` —— **那是另一台机器**（本机不存在 `/home/lux1`，已核实）。后果：

- **§5 里那个 RM-LP 提交包的路径与哈希只存在于本机，你那边读不到。** 不要假设 `outputs/rematch750/RM_LP/seed42/submission/submission.zip` 在你机器上存在。
- 在另一台机器上重建该包**不保证字节一致** —— registry 里 RM_LP 行的 `zip_sha256=db30ff7a…` 与我的重建 `ae654d8f…` 不同，这正是 §9.1 那个账目问题的根因。
- **平台上传不是 agent 的动作**：没有 API，配额属于用户，历史上两次提交都是用户本人完成并回报精确分数的。Codex 能做的是**校验包**与**拿到分数后如实回填**，不是提交。

所以 §5 的正确切分是：**用户负责上传与回报分数；Codex 负责上传前的包校验、判据计算、以及分数回来后的记录与下一步决策。**

### 0.1 2026-09-23 用户新增：算力分工改为「本机出策略、NPU 执行」

**这一节是用户当日的明确指令，优先级高于本文件其它关于分工的描述。**

用户原话要点：

- **本机（`LAPTOP-G42LAJ37`）只负责思考策略**，不再承担长训练。
- 队友的 NPU 承担全部训练；你实测 **≈34.3 分钟/点**，本机 8 轮 ≈80 分钟、16 轮 ≈2.5 小时。
- 我交付的形态改为「**可被直接执行的方案 + 实现**」：config、代码、单测、复现命令、判据齐全，你拿到即可跑。
- **配置（batchsize / epoch / lr 等）参考你 V4 测出的最优配置**，不要两边各写一套。
- **本地得分提升不显著就不占用平台提交名额。**
- 我每交出一个方案会明确告知用户，由用户转给你执行。
- 方案可由我与你**通过本 brief 共同决策**：我在本文件写方案与依据，你给判断，双方以文字留痕，不靠口头。

**因此 §0 的边界要再加一层**：除了「你的机器 vs 我的机器」，现在还多了「**谁跑**」——长训练归 NPU（你那边），我这边只产出方案与实现。§5 的切分（用户上传、你校验与回填）**不变**。

**请你在本 brief 或 V4 文档里显式落两样东西**（我这边现在只能靠猜）：

1. **V4 已实测的最优配置数值**：batchsize / epoch / backbone_lr / head_lr / schedule / 其它实测项。
2. **V4 的轴定义与判据**：A–H 各扫什么、以及提交 `9c11662`（`prune checkpoints without v4 significant local gain`）里那个「显著本地提升」门槛的确切数值。

**另一件需要你确认的重叠**：我另有一条 `clairvoyant/search-mixup-anchor` 分支，上面 4 个点 —— mixup α=0.2 / α=0.4、`feature_distillation_weight` 2.0→0.5（backbone_lr 3e-6→1e-5）、2.0→0.0，均 16 轮。**它们可能与你的 A 轴（mixup）或 B 轴（anchor / 特征蒸馏）重叠**。请判断是**并入你的 V4 编号**还是**关闭**，避免两边重复烧卡。我这边已有实测：α=0.2 本地 micro 72.5000 / macro 71.4466（−0.37pp），α=0.4 为 71.9288 / 70.8878（−0.94pp），8/8 检查点全负，且 train_acc 与 train→val 落差随 α 单调 —— 若你 A 轴的结论与此一致，这条轴可以合并；若不一致，那是重要分歧。

**一处冲突请你一并表态**：§6 的 **P0（RM-LP 探针槽位）**与新增的「本地提升不显著就不占名额」直接冲突 —— 探针的预注册判据恰恰是「**绝对分不判定策略、迁移率才判定**」，它买的就是一个本地看不出提升的测量。我已在 `docs/current_execution_plan.md` 把这条记为「待用户裁决，未决」，你若先看到请给出读法。
...
**动作（分工见 §0）：用户上传 RM-LP 包并回报平台分；Codex 在上传前校验包、拿到分数后按迁移率判据读并回填。**
...
**2026-09-23 起再加一条（§0.1）**：**长训练全部归 NPU（你那边）**，我这边只出策略与实现。我交方案时会明确告知用户，由用户转给你。**先把 §0.1 末尾那两项（V4 实测最优配置数值、A–H 轴定义与显著提升门槛）落到文字上** —— 我按它写 config，否则两边会各写一套。
