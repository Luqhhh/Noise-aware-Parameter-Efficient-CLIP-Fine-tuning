...
| 候选 | 平台分 | 正确数 | 备注 |
|---|---:|---:|---|
| F05 | 65.4711% | 24,515 / 37,444（由 4 位小数分数推断） | 2026-09-24 回填；当前已知 platform leader |
| F03 | 64.41352419613288% | 24,119 / 37,444 | F05 之前的 platform leader |
| FULL01 / RM_FULL | 61.6360% | 约 23,079 / 37,444 | 全量训练重叠诊断，不能与 F05/F03 本地分直接比较 |
| RM_FT | 60.96570879179575% | 22,828 / 37,444 | 独立验证 |
| RM_LT | 60.885589146458706% | 22,798 / 37,444 | 独立验证 |

F05 比 F03 平台高 **+1.0576pp / +396 张**，但 70% 目标需要
**26,211 / 37,444**，F05 还差 **1,696 张**。该回执只有分数，没有
submission ID、带时区时间、reset-period 或包 hash；记录见
`results/rematch750_f05_platform_20260924.json`。F05 本地 macro/micro 为
74.4307% / 75.4435%，但其更重要的新增事实是平台 leader 从 F03 变为 F05。
