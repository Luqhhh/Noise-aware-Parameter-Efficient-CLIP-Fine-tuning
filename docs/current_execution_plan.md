...
四组均固定末轮、使用相同原生224四尺度/Flip双Adapter无校准协议，实际候选attention重新计算；均完成24967行CSV/ZIP校验，包内外CSV字节相同。目录 `outputs/prelim75_v2_20260912/{H0,H1,V0,V1}/submission/`；桌面同哈希副本 `submission_prelim75_{H0,H1,V0,V1}_20260912.zip`。平台未知字段不预填，H0真实65.8629%低于匹配P66.7681%共0.9052pp，不晋级。旧10316行验证完全重叠，只作诊断，不能据表中本地指标确认平台提分或75目标。
...
已执行在线6epoch、缓存head20epoch，4/5候选交付。C判定 `pending_real_platform_feedback`，尚未启动；真实H/V平台胜者均至少67.0681%才组合一次，用V新特征重训H配方，不移植旧head、不平均预测。达到75或既定门槛/预算结束后冻结最佳。只本地提交、不push，不操作平台账号；用户回填平台成绩后继续原条件决策。

## 已移出当前计划

完整上游谱系重建、为复现补齐的校准生产流程、独立环境重新训练及完整复现验收不再是本轮待办或初赛候选交付的阻塞项。它们未完成，未被认证通过；现有代码、来源限制和历史记录保留。未来重新启动需用户明确提出。

上一轮已选实验、交付和成绩回填完成；其候选未实现平台提分，不代表所有优化方向均已完成。H/V工程候选已交付，C未触发；原验证重叠诊断与平台指标继续分开记录。
