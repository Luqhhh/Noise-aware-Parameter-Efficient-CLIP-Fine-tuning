...
9. **（2026-09-26 追加，覆盖第 2/3/6/8 条）改回「自己想策略自己跑」** —— 用户原话：「我之前不是说我本机只需要想策略，队友 npu 负责跑吗，现在改回自己想策略自己跑」。**本机既出策略，也自己执行训练与推理**，不再把方案交给队友的 NPU 跑。随之**失效的前提**（凡引用它们的文档与结论都要一并更正）：
   ...
   - **算力机会成本不再是 ≈0**，回到本机单卡 **RTX 4060 Laptop 8GB**：8 轮 ≈80 分钟、16 轮 ≈2.5 小时串行，**且 16 轮档在本机有 2/2 崩溃史**（`cudaErrorUnknown`，签名见[批次 1 记录](rematch750_search_batch1_20260923.md) §1.4）。因此 GPT-6 brief §12.1「算力机会成本≈0」与 §12.4.5「多种子复现免费 ⇒ 3 seed 配对定为新默认判据」**两条推论一律作废**（已在该文件加 §12.7 更正）。
