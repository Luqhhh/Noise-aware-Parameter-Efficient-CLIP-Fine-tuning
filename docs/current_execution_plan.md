...
## 当前入口：PRELIM75_V8_20260919（已实现，待正式 GPU 队列）
...
代码、配置、队列和来源守卫已接入；36 项定向数值/绑定测试与全套 `538 passed, 1 skipped` 通过，真实资产只读预检确认 103,218 行、500 类均有正有效监督、G0/L1 checkpoint 与回退包哈希匹配，未读取测试预测。唯一跳过项为当前沙箱不可见 CUDA 的既有测试；当前尚未执行两次正式训练集冻结前向、CPU 拟合或测试推理，因此没有 B0/B1 平台候选，不能声称训练源校准有效。固定规格、停止条件和执行入口见 [v8 执行方案](preliminary_75_execution_v8_20260919.md)。
