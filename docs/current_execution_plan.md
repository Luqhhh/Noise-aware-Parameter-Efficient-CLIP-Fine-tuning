...
用户已明确要求恢复执行，并把 batch1024 的精度补偿加入扫描。当前在独立分支 `codex/rematch750_v3_scan` 实施：B64×8、B128×16、B1024×16/LR×4；以同NPU已测B32的71.9047% macro和2870.9秒完整CLI耗时为基准，精度下界71.8047%，达界者选完整耗时最短。旧B1024×8的67.3704%只作历史参照，不重跑。物理NPU0仍不使用。详情及执行状态见[REMATCH750_V3精度/耗时权衡](rematch750_v3_tradeoff_20260923.md)；新增点位尚无实测训练结果或交付包。

以下为上一检查点记录。

## 上一入口：REMATCH750_V2（2026-09-22）

**当时的V3方案记录（已由上方新指令更新）**：[REMATCH750_V3 精度/耗时权衡](rematch750_v3_tradeoff_20260923.md)。原要求仅写方案、暂不运行、不占NPU0；该暂停已解除，非0号空闲设备约束仍有效。原拟比较B64×8与B128×16，现已加入B1024×16/LR×4补偿点。V2实测结论仍有效。
