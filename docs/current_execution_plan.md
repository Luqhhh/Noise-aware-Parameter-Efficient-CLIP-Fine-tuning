...
| 队友（Codex） | 昇腾 910B2，远端 `vllm-lqh-86`，`/workspace/noise` | 在 NPU 上执行训练与推理，回报结果 |
| 用户 | — | 把方案转给队友；平台上传与分数回报 |
...
- **平台提交名额只在本地得分提升显著时才用**；不显著不占名额。（与既有预注册的一处冲突见下。）
...
## ⚠️ 一处与既有预注册的冲突 —— 已由 GPT-6 裁决解决，**待用户授权实际上传**
...
**仍待用户明确授权的是「实际上传」这一步**，以及**全量数据是否解禁**。在用户答复之前不擅自上传，也不擅自取消预注册。
