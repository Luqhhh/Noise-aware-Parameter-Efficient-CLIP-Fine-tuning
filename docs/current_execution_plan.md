# 当前执行入口：REMATCH750_V3 已完成（2026-09-23）

用户要求加入 batch1024 精度补偿、允许使用当时空闲的 NPU0，并每十分钟监控。独立分支 `codex/rematch750_v3_scan` 已完成 B64×8、B128×16、B1024×16/LR×4 扫描及执行审计；B64_LR2 因 B128 达线按预注册规则跳过。以同 NPU B32 的 macro 71.9047%、完整 CLI 2870.9 秒为参照，精度下界为 71.8047%。**B1024×16/LR×4 胜出**：macro 72.3472%、micro 73.4140%、完整 CLI 2055.3 秒，比 B32 macro 高 0.4425pp、完整运行快 1.397 倍；B64 低于精度下界，B128 达线但更慢。已生成 37,444 行 CSV/ZIP，并通过独立提交检查；未上传平台，平台分数未知。完整配置、指标、路径及哈希见[REMATCH750_V3执行记录](rematch750_v3_tradeoff_20260923.md)。
