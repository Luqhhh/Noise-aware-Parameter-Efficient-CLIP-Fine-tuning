...
当前检查点：RM-FT 已完成 8 轮，选中 epoch 8，独立验证 macro 71.8362%、micro 72.9032%，37,444 条预测提交包已校验、待用户上传。优化器/调度器 33,456 次更新对齐，conv1/位置嵌入冻结审计通过。按协作约定本段提交推送后暂停，下一段为从同一 RM-LP 初始化的 RM-LT；RM-FULL 等 FT/LT 平台比较。
