...
## ⚠️ 一处与既有预注册的冲突（需用户裁决，未决）

本机现有 2026-09-22 预注册的 **LP–FT 迁移对照探针**（[预注册](rematch750_lp_ft_transfer_prereg_20260922.md)，brief §5）：其判据明确写的是「**绝对分不判定策略、迁移率才判定**」—— 即那个槽位是**故意**用来买一个「本地看不出提升」的测量的（LP 拿 55% 比 FT 拿 60% 信息量更大）。

新规则第 5 条「本地得分提升不显著就不浪费名额」若直接套用，会**取消这个预注册探针**。两条不能同时成立，需用户明确哪一个优先。**在用户答复之前不擅自上传，也不擅自取消预注册。**
