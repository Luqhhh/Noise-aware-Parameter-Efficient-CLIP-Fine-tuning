...
已执行在线6epoch、缓存head20epoch，4/5候选交付。C判定 `closed_below_combination_gate`，本轮取消：H0/H1均低于P，H路线关闭，不追加重拟合或扫描；V0/V1平台待回填后保留真实胜者或关闭V。达到75或既定门槛/预算结束后冻结最佳。只本地提交、不push，不操作平台账号；用户回填V0/V1平台成绩后继续原保留/关闭决策。
