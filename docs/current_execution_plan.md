...
## 本轮：按用户授权顺序执行，H0/H1/V0已交付，V1训练中
...
H0/H1已完成真实诊断及CSV/ZIP校验，H段交接提交491c7f9；当前顺序执行V0/V1，不以用户瓶颈判断为条件；C仍须真实H/V平台分数均满足门槛。每段须携实际检查结果交接，不把缓存或接线写成实验完成。只在main本地提交，不push；平台上传及成绩回填由用户完成。
...
执行截面（2026-09-13）：V0第1/3 epoch运行中，首步视觉/head梯度非零，O3/PTA梯度为零，冻结参数无泄漏；batch32未降档。运行源码冻结于 `outputs/prelim75_v2_20260912/training_source_visual/`。记录不替代完成检查；V0/V1尚无交付或平台分数。

最新截面：V0已完成3epoch、真实诊断及24967行提交校验；重叠raw/clean-core相对P分别+2.7142pp/+2.8918pp，线上未知。V0包已放桌面。V1已启动，local-only loss对视觉投影/O3/PTA的梯度均非零，batch32未降档。C仍等待真实H/V平台分数，不从本地诊断触发。

真实平台更新：H0=65.8629%，相对P66.7681%为-0.9052pp，不晋级。H1待测，H路线尚未整体裁决；V1继续，C仍按真实H/V门槛执行。
