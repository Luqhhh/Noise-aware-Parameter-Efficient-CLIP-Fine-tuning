# 当前执行入口：提分搜索 —— 本机只出策略、NPU 执行（2026-09-23 起）
...
8. **（2026-09-23 追加）用哪个提交包也由队友决定** —— 原话「之后用哪个提交包也都由队友决定，你只需要想策略就行了，他 npu 跑的很快而且可以并行」。因此：**本机不决定跑什么、不决定提交哪个包、不做平台上传**；凡是「值不值得花一个名额」的机会成本判断归队友。队友 NPU 单点 ≈34.3 分钟且**可并行** ⇒ **算力机会成本 ≈ 0**，真正稀缺的是**每天 2 次提交名额**与人的注意力。第 5 条「本地提升不显著就不浪费名额」现在是队友的规则，本机只负责把判据和候选摆清楚。
...
判据只有一条：`RM_FULL` 的平台分相对 RM-FT **60.96570879179575%** 的差。**本轮本地不存在可信读数**（val ⊂ train，流程会把 `validation_status` 标为 `overlapping diagnosis`），且**预期落在 ±0.20pp 空档** —— 它关的是「数据量」这条轴（复赛唯一未喂给模型的 11%），不是期待涨分。因此它要花一个平台名额买信息 —— **跑不跑、要不要占名额由队友裁决**（算力机会成本已≈0，FULL00+FULL01 合计仍是 ≈34 分钟量级且可与其他点位并行）。NPU 执行与验收见[预注册](rematch750_full_ft_prereg_20260923.md)。

## 分工落实

| 角色 | 机器 | 职责 |
|---|---|---|
| clairvoyanttt（我） | `LAPTOP-G42LAJ37`，RTX 4060 Laptop 8GB / WSL | **出策略、写具体实现**（config + 代码 + 单测 + 复现命令 + 判据），提交到 `成员名/方案名` 分支；**不再新开长训练**；及时 `git fetch origin` 拉队友数据 |
| 队友（Codex） | 昇腾 910B2，远端 `vllm-lqh-86`，`/workspace/noise` | **决定跑什么**；在 NPU 上执行训练与推理（可并行）；**决定用哪个提交包并上传平台**；回报结果与分数 |
| 用户 | — | 转达双方方案；对策略方向裁决 |

## 本阶段因此改变的几点

- **本机 GPU 不再新开长训练。** B2 重跑（2026-09-23 16:35:13 启动，约 19:05 训完）是本机最后一批长训练。**B1 不在本机重跑**（它 15:03 崩溃后无任何数据），改为并入交给 NPU 的点位 —— 在 NPU 上跑一次 34 分钟，比在本机赌 2.5 小时划算。
...
## ⚠️ 一处与既有预注册的冲突 —— 已由 GPT-6 裁决；**实际上传现归队友决定**
...
**2026-09-23 更新**：全量数据已解禁（见上方在途方案）。「实际上传」这一步现归**队友**决定（见总纲第 8 条）—— 本机不做上传、不做名额决策，只负责把判据与候选摆清楚；**不擅自上传，也不擅自取消既有预注册**。
