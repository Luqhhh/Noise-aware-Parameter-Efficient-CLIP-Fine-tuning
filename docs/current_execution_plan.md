# 当前执行入口：REMATCH750_V3 扫描实施（2026-09-23）

用户已明确要求恢复执行，并把 batch1024 的精度补偿加入扫描；随后明确删除“不使用NPU0”的限制。当前在独立分支 `codex/rematch750_v3_scan` 实施：B64×8、B128×16、B1024×16/LR×4；以同NPU已测B32的71.9047% macro和2870.9秒完整CLI耗时为基准，精度下界71.8047%，达界者选完整耗时最短。旧B1024×8的67.3704%只作历史参照，不重跑。只选当时空闲的NPU。B64已完成并通过执行审计：macro70.8299%、完整CLI1946.7秒，精度未达线；B128已完成并通过审计：macro71.9320%达线但完整CLI3663.7秒慢于B32；B1024正在运行，条件B64_LR2按原规则跳过。暂无V3交付包或平台成绩。详情及执行状态见[REMATCH750_V3精度/耗时权衡](rematch750_v3_tradeoff_20260923.md)。
