...
主表保留整体指标。固定尾部75类按训练支持度、class ID预先确定；其余675类采用补集。各模型自身最差75类另列，类别集合不保证相同。旧训练未采集可直接比较的纯训练耗时，留空，不用吞吐短窗口估算补值。

| candidate | selected_epoch | raw_macro | raw_micro | 固定训练尾部75类 macro | 其余675类 macro | successful_optimizer_updates | 训练耗时 | 平台分数 |
|---|---:|---:|---:|---:|---:|---:|---|---:|
| RM_FT | 8 | 71.8362% | 72.9032% | 55.5827% | 73.6421% | 33,456 | 未记录 | 60.9657088% |
| RM_LT | 8 | 71.8655% | 72.6949% | 58.1485% | 73.3897% | 33,456 | 未记录 | 60.8855891% |

各自最差75类 macro：RM_FT 22.5567%，RM_LT 23.5166%；不用于替代整体晋级指标。REMATCH750_V2 新候选主表与成功更新审计见 [V2执行记录](rematch750_v2_execution_20260922.md)。

## 平台结果

用户于 2026-09-22 回报两次既有单模型提交的精确得分：RM_FT 为 **60.96570879179575%（22,828 / 37,444）**，RM_LT 为 **60.885589146458706%（22,798 / 37,444）**。RM_FT 高 **0.080119645pp / 30 张**，因此平台总体指标选择 RM_FT；本地 RM_LT 的 macro 与尾部优势没有转化成平台总体准确率优势。策略映射、ZIP SHA 与回执缺失字段见 [平台结果记录](rematch750_platform_results_20260922.md)。这两个分数不属于 RM_OOFW。
