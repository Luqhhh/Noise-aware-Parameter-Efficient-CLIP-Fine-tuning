...
状态：**实现与只读资产 preflight 已完成；D0=1,532/2,048、smoke 与重叠诊断通过。首轮完整重放因误用训练 GPU 初始化而关闭 cuDNN TF32，产生 32 行 L1 不一致并按守卫停止，未交付候选；A/B 已确认并修复为归档通用推理数值协议，等待保留失败证据后的固定重跑。**
