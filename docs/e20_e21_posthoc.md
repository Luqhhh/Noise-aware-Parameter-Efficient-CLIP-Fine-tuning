...
- **E21 已提交，平台仍在评测**。它与 E20 的测试集预测有 1,660/24,967
  （6.649%）不同，具备独立评测价值，但在平台结果返回前不能宣称提升。
...
2. E21 返回后先登记精确平台分数，再与 E20 和团队最佳进行同一口径比较。
3. 若 E21 未超过 60.4758%，继续优先验证与 E20 有更大预测差异、且严格本地门禁更强的
   后续候选；不得根据训练内分数选择全量训练超参数。
