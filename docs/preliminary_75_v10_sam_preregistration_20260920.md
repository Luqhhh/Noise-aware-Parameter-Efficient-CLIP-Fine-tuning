...
本轮不回退到 V5X/O2L（平台 `64.0806%`），不继续 v8 训练源偏置（`67.8816% / 62.0659%`），也不扫描 v9 原图回采样参数。v9 R0/R1 的平台成绩仍待回填，但其唯一变量是冻结推理像素来源，与本轮优化器机制正交。
...
1. v9 R0/R1 结果若已回填，先同步并记录，但不得据此改动本协议；
