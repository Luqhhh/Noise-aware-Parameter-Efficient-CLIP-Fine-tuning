...
- date: 2026-09-25
- status: **三个候选均已完成并落表（2026-09-26）；全部为 fail/weak，未过 +0.30pp 晋级门，
  无平台提交**
...
| NEW03 集合监督 | 3 | 0.7543330 / 0.7648522 | 0.7588806 / 0.7670027 | **+0.0615pp** | 199 / 167 | weak |

参照：L05 winner 的 decode macro/micro = 0.7582651 / 0.7665322；`delta_vs_l05` 是 decode macro
相对该值的百分点差。本地 +0.30pp 晋级门未过，三个候选都没有理由替换既有 L05 winner；
也没有生成测试候选或上传平台。

结论：单次 3-epoch 短续训的**整套续训配方**相对 L05 winner 的净收益是
NEW01 −0.03pp / NEW02 +0.01pp / NEW03 +0.06pp（decode macro）。按预注册口径，这只能记为
「阶段二续训配方收益」，不能归因于某个新组件；三个方向在本预算下都未达门。
NEW03 是三者中唯一在固定协议下同时提高 macro 与 micro 的（+0.0615pp / +0.0470pp），
但幅度只有门槛的约 1/5，不足以支撑提交。
...
队列状态：2026-09-25 22:16 在本机 RTX 4070（单卡、串行）启动，2026-09-26 14:41 全部结束。
中途两次非结果性中断已处理：NEW02 第一次在 epoch 3 被外部进程组清理杀掉（无 OOM、无 traceback），
保留为 `NEW02_interrupted_0524/` 并从头重跑；NEW03 第一次因集合质量下溢缺陷在 epoch 1 第一步
失败，修复（钳制）后重跑。`results/l05_new_candidates.csv` 现有三行完整结果，见上方「结果」表。
