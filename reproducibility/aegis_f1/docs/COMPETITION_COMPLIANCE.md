...
本矩阵把赛规转成可执行约束。若后续官方通知与该 PDF 冲突，以最新官方通知为准，并同步更新配置和审计。2026-09-15 最新通知已将复赛和半决赛类别数分别减半为 750 和 500。
...
| 复赛/半决赛需分别适应 750/500 类长尾数据 | 类别数和样本数均配置化；提供训练期类先验校正，但默认关闭，必须单独消融 | `class_prior_adjustment_tau`、macro 选择指标 |
